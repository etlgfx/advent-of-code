...
function exercise1(input) {
    let gamma = [];
    let epsilon = [];
    input.map((code) => code.split('').map((c) => Number(c))).reduce((prev, cur) => {
        for (let i = 0; i < prev.length; i++) {
            prev[i] += cur[i];
        }

        return prev;
    }, new Array(12).fill(0)).forEach((elem) => {
        gamma.push(elem > input.length / 2 ? 1 : 0);
        epsilon.push(elem < input.length / 2 ? 1 : 0);
    });

    gamma = Number.parseInt(gamma.join(''), 2);
    epsilon = Number.parseInt(epsilon.join(''), 2);

    return {gamma: gamma, epsilon: epsilon, result: gamma * epsilon};
}

console.log('exercise1', exercise1(input));
...
function recurse(inp, position, o2) {
    if (inp.length == 1) {
        return inp[0];
    }
    ...
    inp.forEach((ele) => {
        let n = Number(ele.charAt(position));
    ...
    if (ones.length >= inp.length / 2) {
        if (o2) {
            return recurse(ones, position + 1, o2);
        }
        else {
            return recurse(zeros, position + 1, o2);
        }
    }
    else {
        if (o2) {
...
function exercise2(input) {
    let o2 = Number.parseInt(recurse(input, 0, true), 2);
    let co2 = Number.parseInt(recurse(input, 0, false), 2);

    console.log(o2, co2, o2 * co2);
}

console.log('exercise2', exercise2(input));
